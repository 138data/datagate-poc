...
/**
 * 添付直送機能が有効かどうか判定
 * @param {string} recipientDomain - 受信者のドメイン（例: @138io.com）
 * @returns {boolean}
 */
export function canUseDirectAttach(recipientDomain) {
  // 機能が無効の場合
  ...
  const normalizedDomain = recipientDomain.toLowerCase();
  return allowedDomains.some(allowed => normalizedDomain.endsWith(allowed));
}