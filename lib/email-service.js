...
async function sendEmail({ to, fileId, fileName, otp, shouldAttach = false, fileBuffer = null }) {
  ...
  console.log('[DEBUG] Calling sendDownloadLinkEmail with:', { to, fileId, fileName });
  
  try {
    if (shouldAttach && fileBuffer) {
      console.log('[DEBUG] Sending as attachment');
      ...
      console.log('[DEBUG] Sending download link');
      const result = await sendDownloadLinkEmail({ to, fileId, fileName });
      console.log('[DEBUG] Download link result:', result);
      return { sent: true, success: result.success, mode: 'link', reason: null };
    }
  } catch (error) {
    console.error('[ERROR] sendEmail failed:', error);
    return { sent: false, success: false, mode: 'link', reason: error.message };
  }
}
