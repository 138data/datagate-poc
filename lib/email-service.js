 ...
 * @param {string} data.recipientEmail - 受信者メールアドレス
 * @param {string} data.fileName - ファイル名
 * @param {number} data.fileSize - ファイルサイズ
 * @param {string} data.downloadUrl - ダウンロードURL
 * @param {string} data.otp - 6桁のOTP
 * @param {string} data.expiresAt - 有効期限
 * @returns {Promise<Object>} 送信結果
 */
export async function sendOTPEmail(to, data) {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
    ...
    <div style="background: white; border: 2px solid #667eea; border-radius: 8px; padding: 20px; margin: 20px 0;">
      <p style="margin: 5px 0;"><strong>ファイル名:</strong> ${data.fileName}</p>
      <p style="margin: 5px 0;"><strong>ファイルサイズ:</strong> ${formatFileSize(data.fileSize)}</p>
      <p style="margin: 5px 0;"><strong>有効期限:</strong> ${new Date(data.expiresAt).toLocaleString('ja-JP')}</p>
    </div>
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${data.downloadUrl}" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px 40px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">📥 ダウンロードページを開く</a>
    </div>
    ...
    <div style="background: #fff3cd; border-left: 4px solid #ffc107; padding: 15px; margin: 20px 0; border-radius: 4px;">
      <p style="margin: 0; font-size: 14px; color: #856404;">
        <strong>⚠️ 重要:</strong> この OTP は一度のみ有効です。第三者に教えないでください。
      </p>
    </div>
    ...
    <div style="background: #e7f3ff; border-left: 4px solid #667eea; padding: 15px; margin: 20px 0; border-radius: 4px;">
      <p style="margin: 0; font-size: 13px; color: #004085;">
        ...
        • ダウンロードは3回まで可能です<br>
        • ${Math.floor((new Date(data.expiresAt) - new Date()) / (1000 * 60 * 60 * 24))}日後に自動削除されます<br>
        • パスワードの入力回数制限があります
      </p>
    </div>
