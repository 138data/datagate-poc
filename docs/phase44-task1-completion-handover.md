# Phase 44 Task 1-1/1-2 完了引き継ぎ

**作成**: 2025-11-05  
**ブランチ**: feature/phase44-reco-robustness  
**状態**: Task 1-1, 1-2 完了

## 完了内容

### Task 1-1: 推奨サイズ計算改善
- ファイル: lib/policy-analytics.js
- 入力サニタイゼーション、外れ値除去、フォールバック戦略
- 0.0MB問題を修正

### Task 1-2: 統計UIエラーハンドリング
- ファイル: admin/index.html  
- API契約名統一、null安全化
- コンソールエラー修正

## 次のステップ

### 選択肢A: テスト・デプロイ（推奨）
```powershell
vercel
git checkout main
git merge feature/phase44-reco-robustness
vercel --prod
```

### 選択肢B: Task 2（A/Bテスト）実装
所要: 2-3時間

## 次回セッション開始例
>> ```
>> Phase 44 Task 1-1/1-2 完了の引き継ぎを確認しました。
>> 選択肢A（テスト・デプロイ）を実行します。
>>
>> ```powershell
>> Set-Location D:\datagate-poc
>> git status
>> vercel
>> ```
>> ```
>> "@
[System.IO.File]::WriteAllText("D:\datagate-poc\docs\phase44-task1-completion-handover.md", /**
 * Policy Analytics - 統計データに基づくポリシー推奨値の算出
 * Phase 44-Task 1-1: 推奨サイズ計算ロジックの堅牢化
 *
 * 改善内容:
 * - 入力サニタイゼーション（Number.isFinite フィルタ）
 * - 外れ値除去（IQR法）
 * - フォールバック戦略（N<30 → デフォルト値）
 * - 最小/最大ガードレール（1MB～25MB）
 * - 信頼度計算（high/medium/low）
 */

import { kv } from '@vercel/kv';

// 定数定義
const DEFAULT_MAX_SIZE = 4718592; // 4.5MB（デフォルト値）
const MIN_RECOMMENDED_SIZE = 1048576; // 1MB（最小値）
const MAX_RECOMMENDED_SIZE = 26214400; // 25MB（最大値）
const MIN_SAMPLES_FOR_HIGH_CONFIDENCE = 30; // 高信頼度に必要な最小サンプル数
const MIN_SAMPLES_FOR_MEDIUM_CONFIDENCE = 10; // 中信頼度に必要な最小サンプル数

/**
 * ファイルサイズのパーセンタイル計算
 * @param {Array<number>} sizes - ファイルサイズの配列（バイト）
 * @param {number} percentile - パーセンタイル（0-100）
 * @returns {number} パーセンタイル値
 */
function calculatePercentile(sizes, percentile) {
  if (sizes.length === 0) return 0;

  const sorted = [...sizes].sort((a, b) => a - b);
  const index = Math.ceil((percentile / 100) * sorted.length) - 1;
  return sorted[Math.max(0, index)];
}

/**
 * IQR法による外れ値除去
 * @param {Array<number>} sizes - ファイルサイズの配列
 * @returns {Array<number>} 外れ値を除去したサイズ配列
 */
function removeOutliers(sizes) {
  if (sizes.length < 4) return sizes; // サンプルが少なすぎる場合はそのまま

  const q1 = calculatePercentile(sizes, 25);
  const q3 = calculatePercentile(sizes, 75);
  const iqr = q3 - q1;
  const lowerBound = q1 - (1.5 * iqr);
  const upperBound = q3 + (1.5 * iqr);

  return sizes.filter(size => size >= lowerBound && size <= upperBound);
}

/**
 * サイズデータのサニタイゼーション
 * @param {any} size - サイズ値
 * @returns {number|null} サニタイズされたサイズ値、無効な場合は null
 */
function sanitizeSize(size) {
  // 数値に変換
  const numSize = Number(size);
  
  // 有限数かつ正の値のみ受け入れる
  if (!Number.isFinite(numSize) || numSize <= 0) {
    return null;
  }
  
  return numSize;
}

/**
 * 信頼度の計算
 * @param {number} sampleCount - サンプル数
 * @returns {string} 信頼度レベル（'high'/'medium'/'low'）
 */
function calculateConfidence(sampleCount) {
  if (sampleCount >= MIN_SAMPLES_FOR_HIGH_CONFIDENCE) {
    return 'high';
  } else if (sampleCount >= MIN_SAMPLES_FOR_MEDIUM_CONFIDENCE) {
    return 'medium';
  } else {
    return 'low';
  }
}

/**
 * 監査ログからファイルサイズデータを抽出
 * @param {number} days - 分析対象の日数
 * @returns {Promise<Array<Object>>} ファイルサイズデータ
 */
async function extractFileSizeData(days = 7) {
  try {
    const now = Date.now();
    const cutoffTime = now - (days * 86400000);

    // 監査ログのキーを取得
    const keys = await kv.keys('audit:*');
    const sizeData = [];

    for (const key of keys) {
      const log = await kv.get(key);
      if (!log) continue;

      // 期間内のログのみ対象
      if (log.timestamp < cutoffTime) continue;

      // サイズのサニタイゼーション
      const sanitizedSize = sanitizeSize(log.size);
      if (sanitizedSize !== null) {
        sizeData.push({
          size: sanitizedSize,
          mode: log.mode,
          reason: log.reason,
          timestamp: log.timestamp
        });
      }
    }

    return sizeData;
  } catch (error) {
    console.error('Failed to extract file size data:', error);
    return [];
  }
}

/**
 * ファイルサイズ分布の分析
 * @param {number} days - 分析対象の日数
 * @param {boolean} removeOutliersFlag - 外れ値除去フラグ
 * @returns {Promise<Object>} 分析結果
 */
export async function analyzeFileSizeDistribution(days = 7, removeOutliersFlag = true) {
  try {
    const sizeData = await extractFileSizeData(days);

    if (sizeData.length === 0) {
      return {
        totalFiles: 0,
        filteredFiles: 0,
        p50: 0,
        p75: 0,
        p95: 0,
        p99: 0,
        max: 0,
        average: 0,
        confidence: 'low'
      };
    }

    let sizes = sizeData.map(d => d.size);
    const originalCount = sizes.length;

    // 外れ値除去
    if (removeOutliersFlag) {
      sizes = removeOutliers(sizes);
    }

    const total = sizes.reduce((a, b) => a + b, 0);
    const confidence = calculateConfidence(sizes.length);

    return {
      totalFiles: originalCount,
      filteredFiles: sizes.length,
      p50: calculatePercentile(sizes, 50),
      p75: calculatePercentile(sizes, 75),
      p95: calculatePercentile(sizes, 95),
      p99: calculatePercentile(sizes, 99),
      max: Math.max(...sizes),
      average: Math.round(total / sizes.length),
      confidence
    };
  } catch (error) {
    console.error('Failed to analyze file size distribution:', error);
    throw error;
  }
}

/**
 * 添付直送の成功率分析
 * @param {number} days - 分析対象の日数
 * @returns {Promise<Object>} 成功率データ
 */
export async function analyzeDirectAttachSuccessRate(days = 7) {
  try {
    const sizeData = await extractFileSizeData(days);

    if (sizeData.length === 0) {
      return {
        totalAttempts: 0,
        successCount: 0,
        successRate: 0,
        fallbackReasons: {}
      };
    }

    const directAttachAttempts = sizeData.filter(d =>
      d.mode === 'attach' || d.mode === 'link'
    );

    const successCount = sizeData.filter(d => d.mode === 'attach').length;
    const fallbackCount = sizeData.filter(d => d.mode === 'link').length;

    // フォールバック理由の集計
    const fallbackReasons = {};
    sizeData.forEach(d => {
      if (d.mode === 'link' && d.reason) {
        fallbackReasons[d.reason] = (fallbackReasons[d.reason] || 0) + 1;
      }
    });

    return {
      totalAttempts: directAttachAttempts.length,
      successCount,
      fallbackCount,
      successRate: directAttachAttempts.length > 0
        ? Math.round((successCount / directAttachAttempts.length) * 100)
        : 0,
      fallbackReasons
    };
  } catch (error) {
    console.error('Failed to analyze direct attach success rate:', error);
    throw error;
  }
}

/**
 * 推奨サイズの計算（堅牢版）
 * @param {Object} sizeDistribution - サイズ分布データ
 * @returns {number} 推奨サイズ（バイト）
 */
function calculateRecommendedSize(sizeDistribution) {
  const { p95, p75, confidence, filteredFiles } = sizeDistribution;

  // ケース1: サンプル不足（N<10）→ デフォルト値
  if (filteredFiles < MIN_SAMPLES_FOR_MEDIUM_CONFIDENCE) {
    return DEFAULT_MAX_SIZE;
  }

  // ケース2: P95が0または無効 → P75を使用、それも無効ならデフォルト値
  let baseSize = p95;
  if (!baseSize || baseSize <= 0) {
    baseSize = p75;
    if (!baseSize || baseSize <= 0) {
      return DEFAULT_MAX_SIZE;
    }
  }

  // ケース3: 信頼度が低い場合は余裕を持たせる（+20%）
  const margin = confidence === 'low' ? 1.2 : 1.1;
  let recommendedSize = Math.ceil(baseSize * margin);

  // ガードレール適用
  recommendedSize = Math.max(MIN_RECOMMENDED_SIZE, recommendedSize);
  recommendedSize = Math.min(MAX_RECOMMENDED_SIZE, recommendedSize);

  return recommendedSize;
}

/**
 * 推奨ポリシー値の算出
 * @param {number} days - 分析対象の日数
 * @returns {Promise<Object>} 推奨ポリシー値
 */
export async function calculateRecommendedPolicy(days = 7) {
  try {
    const [sizeDistribution, successRate] = await Promise.all([
      analyzeFileSizeDistribution(days),
      analyzeDirectAttachSuccessRate(days)
    ]);

    // 推奨サイズ閾値の計算（堅牢版）
    const recommendedMaxSize = calculateRecommendedSize(sizeDistribution);

    // サイズ超過によるフォールバックが多い場合は閾値を上げる提案
    const sizeExceededCount = 
      (successRate.fallbackReasons['size_over_threshold'] || 0) +
      (successRate.fallbackReasons['size_exceeded'] || 0); // 後方互換
    const shouldIncreaseSize = sizeExceededCount > (successRate.totalAttempts * 0.1);

    // 添付直送の有効化推奨判定
    // 成功率が80%以上なら有効化を推奨
    const recommendEnableDirectAttach = successRate.successRate >= 80;

    // 推奨値の説明文生成
    const sizeReasonParts = [];
    if (sizeDistribution.confidence === 'low') {
      sizeReasonParts.push(`サンプル数が少ない（${sizeDistribution.filteredFiles}件）ため、デフォルト値をベースに推奨`);
    } else if (sizeDistribution.p95 === 0) {
      sizeReasonParts.push('データ不足のため、デフォルト値を推奨');
    } else {
      const p95MB = Math.round(sizeDistribution.p95 / 1048576 * 10) / 10;
      sizeReasonParts.push(`P95(${p95MB}MB)を基準に算出`);
      if (shouldIncreaseSize) {
        sizeReasonParts.push(`サイズ超過が${sizeExceededCount}件発生`);
      }
    }

    return {
      analysis: {
        period: `${days}日間`,
        totalFiles: sizeDistribution.totalFiles,
        filteredFiles: sizeDistribution.filteredFiles,
        confidence: sizeDistribution.confidence,
        sizeDistribution: {
          p50: sizeDistribution.p50,
          p75: sizeDistribution.p75,
          p95: sizeDistribution.p95,
          p99: sizeDistribution.p99,
          average: sizeDistribution.average
        },
        successRate: {
          total: successRate.totalAttempts,
          success: successRate.successCount,
          fallback: successRate.fallbackCount,
          rate: successRate.successRate,
          fallbackReasons: successRate.fallbackReasons
        }
      },
      recommendations: {
        enableDirectAttach: recommendEnableDirectAttach,
        directAttachMaxSize: recommendedMaxSize,
        confidence: sizeDistribution.confidence,
        reason: {
          enableDirectAttach: recommendEnableDirectAttach
            ? `成功率${successRate.successRate}%で安定しています`
            : `成功率${successRate.successRate}%のため、改善が必要です`,
          directAttachMaxSize: sizeReasonParts.join('。')
        }
      },
      insights: generateInsights(sizeDistribution, successRate)
    };
  } catch (error) {
    console.error('Failed to calculate recommended policy:', error);
    throw error;
  }
}

/**
 * インサイトの生成
 * @param {Object} sizeDistribution - サイズ分布データ
 * @param {Object} successRate - 成功率データ
 * @returns {Array<string>} インサイトメッセージ
 */
function generateInsights(sizeDistribution, successRate) {
  const insights = [];

  // データ量のインサイト（最優先）
  if (sizeDistribution.filteredFiles < MIN_SAMPLES_FOR_MEDIUM_CONFIDENCE) {
    insights.push(`ℹ️ データ量が少ない（${sizeDistribution.filteredFiles}件）ため、推奨値の精度が限定的です。`);
  } else if (sizeDistribution.confidence === 'medium') {
    insights.push(`ℹ️ データ量は十分（${sizeDistribution.filteredFiles}件）ですが、さらにデータが蓄積されると精度が向上します。`);
  } else if (sizeDistribution.confidence === 'high') {
    insights.push(`✅ 十分なデータ量（${sizeDistribution.filteredFiles}件）により、高精度な推奨値を算出しています。`);
  }

  // 外れ値除去のインサイト
  const removedCount = sizeDistribution.totalFiles - sizeDistribution.filteredFiles;
  if (removedCount > 0) {
    insights.push(`📊 ${removedCount}件の外れ値を除外して分析しています。`);
  }

  // サイズ分布のインサイト
  if (sizeDistribution.p95 > 5242880) { // 5MB
    insights.push('⚠️ P95サイズが5MBを超えています。大容量ファイルの利用が多い傾向です。');
  }

  if (sizeDistribution.average < 1048576 && sizeDistribution.average > 0) { // 1MB
    insights.push('✅ 平均ファイルサイズが1MB未満で、軽量ファイルが中心です。');
  }

  // 成功率のインサイト
  if (successRate.successRate < 70) {
    insights.push('⚠️ 添付直送の成功率が70%未満です。閾値の見直しを推奨します。');
  } else if (successRate.successRate >= 90) {
    insights.push('✅ 添付直送の成功率が90%以上で、非常に良好です。');
  }

  // フォールバック理由のインサイト
  const topReason = Object.entries(successRate.fallbackReasons)
    .sort((a, b) => b[1] - a[1])[0];

  if (topReason && successRate.totalAttempts > 0) {
    const [reason, count] = topReason;
    const percentage = Math.round((count / successRate.totalAttempts) * 100);
    
    // reason の日本語化
    const reasonMap = {
      'domain_not_allowed': 'ドメイン不許可',
      'size_over_threshold': 'サイズ超過',
      'size_exceeded': 'サイズ超過',
      'sandbox_link_forced': 'サンドボックスモード',
      'feature_disabled': '機能無効',
      'default_policy_link': 'デフォルトポリシー'
    };
    const reasonJa = reasonMap[reason] || reason;
    
    insights.push(`📊 フォールバックの主要因は「${reasonJa}」(${percentage}%)です。`);
  }

  return insights;
}

/**
 * ポリシー推奨APIレスポンスの生成
 * @param {number} days - 分析対象の日数
 * @returns {Promise<Object>} APIレスポンス形式の推奨データ
 */
export async function getPolicyRecommendations(days = 7) {
  try {
    const recommended = await calculateRecommendedPolicy(days);

    return {
      success: true,
      analysis: recommended.analysis,
      recommendations: {
        ...recommended.recommendations,
        unit: 'bytes' // 単位を明示
      },
      insights: recommended.insights,
      generatedAt: Date.now()
    };
  } catch (error) {
    console.error('Failed to get policy recommendations:', error);
    return {
      success: false,
      error: error.message,
      generatedAt: Date.now()
    };
  }
}, (New-Object System.Text.UTF8Encoding False))
git add docs/phase44-task1-completion-handover.md
git commit -m "docs(phase44): Add Task 1-1/1-2 completion handover"
git push origin feature/phase44-reco-robustness
Write-Host "引き継ぎ資料作成完了" -ForegroundColor Green